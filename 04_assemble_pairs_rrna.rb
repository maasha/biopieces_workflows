...
# Read in cleaned ribosomal RNA matching reads and assemble read pairs allowing
# for unassembled reads and write a table of IDs for read pairs that were
# assembled.
#
...
Parallel.each(samples, in_processes: cpus) do |sample|
  ...
  BP.new.
  read_fastq(input: File.join("clean_rrna", "#{sample[0]}_clean.fq"), encoding: :base_33).
...
Parallel.each(samples, in_processes: cpus) do |sample|
  ...
  write_fasta(output: "#{sample[0]}_rrna_assemble_pairs_pairs.fna", force: true).
...
Parallel.each(samples, in_processes: cpus) do |sample|
  ...
  write_fasta(output: "#{sample[0]}_rrna_assemble_pairs_singletons.fna", force: true).
